...
console.log('Loading daily electricity price data...');
const ePriceCsv = readCSV('e_price.csv');
const times = ePriceCsv.time || [];
...
// 获取价格数据列名
const tazIds = Object.keys(ePriceCsv).filter(k => !isNaN(k) && k !== 'time');
...
for (const [date, rows] of Object.entries(dateRowIndices)) {
    ...
    for (const tazId of tazIds) {
        const dailyHours = rows.map(r => ePriceCsv[tazId]?.[r] || 0.76);
...
// 保存
const outputPath = path.join(dataPath, 'daily-e-prices.js');
const output = `window.DAILY_E_PRICES = ${JSON.stringify(dailyData)};`;
